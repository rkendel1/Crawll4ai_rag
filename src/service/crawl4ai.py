...
async def crawl_recursive_internal_links(crawler: AsyncWebCrawler, start_urls: List[str], max_depth: int = 3, max_concurrent: int = 10, results_all : List[Any] = None) -> List[Dict[str, Any]]:
    """
    ...
    for depth in range(max_depth):
        ...
        results_all.append(( await crawler.arun_many(urls=urls_to_crawl, config=crawl_config, dispatcher=dispatcher)))
    
    return [{'url': r.url, 'markdown': r.markdown} for r in results_all if r.success and r.markdown]