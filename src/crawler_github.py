...
if __name__ == '__main__':
    ...
    test_repo_url_small = "https://github.com/jerryjliu/llama_index" 
    # test_repo_url_small = "https://github.com/openai/openai-cookbook"
    # test_repo_url_small = "https://github.com/github-copilot/example-javascript"


    async def mock_broadcast(message: Dict[str, Any]):
        print(f"MOCK BROADCAST: {message}")

    async def main_async_zip_test():
        print(f"\n--- Testing crawl_github_repository_async (ZIP Download Method) ---")
        
        test_output_dir = "EXPORT_GITHUB_TEST"
        ...
        if github_auth_token:
            ...
            print("GITHUB_TOKEN not found in environment. API calls may be rate-limited or fail for private repos.")

        crawled_files_async = await crawl_github_repository_async(
            repo_url=test_repo_url_small,
            branch_name=None, # Test with default branch
            broadcast_progress=mock_broadcast,
            save_raw_content=True, 
            output_dir_for_raw=test_output_dir,
            github_token=github_auth_token
        )
        
        print(f"\nAsync ZIP crawl results ({len(crawled_files_async)} files processed):")
        for f_idx, f_async in enumerate(crawled_files_async):
            if f_idx < 5 or f_idx > len(crawled_files_async) - 6 : 
                status = "OK" if f_async.get('success') else f"FAIL ({f_async.get('error_message')})"
                content_preview = str(f_async.get('content', ''))[:70].replace('\n', ' ') + "..." if f_async.get('content') and isinstance(f_async.get('content'), str) else "N/A (or binary)"
                print(f"ASYNC: {f_async['title']} ({f_async.get('api_path', 'N/A')}) - Status: {status}, Type: {f_async.get('content_type')}, Content: {content_preview}")
            elif f_idx == 5:
                print("...")
        
        if not crawled_files_async:
            print("No files were processed.")
        
        # Test cleanup
        # print("\n--- Testing with save_raw_content=False (should cleanup) ---")
        # if os.path.exists(test_output_dir): shutil.rmtree(test_output_dir)
        # os.makedirs(test_output_dir, exist_ok=True)
        # await crawl_github_repository_async(
        #     repo_url=test_repo_url_small,
        #     branch_name="main", 
        #     broadcast_progress=mock_broadcast,
        ...
        #     github_token=github_auth_token
        # )
        # print(f"Check if {test_output_dir} and its subdirectories are cleaned up (if save_raw_content was False).")


    asyncio.run(main_async_zip_test())