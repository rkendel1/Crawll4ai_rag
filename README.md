...
# OpenRouter Configuration (optional - set USE_OPENROUTER=true to enable)
...
The server supports both OpenAI and OpenRouter for LLM operations (contextual embeddings, code summaries, and source summaries):

- **OpenAI** (default): Uses OpenAI's API directly. Set `MODEL_CHOICE` to any OpenAI model (e.g., `gpt-4.1-nano`, `gpt-4`, `gpt-3.5-turbo`).

- **OpenRouter**: Access a wide variety of models through OpenRouter's unified API. To use OpenRouter:
  1. Set `USE_OPENROUTER=true`
  2. Provide your `OPENROUTER_API_KEY`
  3. Set `MODEL_CHOICE` to any model available on OpenRouter (e.g., `anthropic/claude-3-opus`, `google/gemini-pro`, `meta-llama/llama-3-70b-instruct`)

**Note**: OpenRouter is only used for LLM operations. Embeddings always use OpenAI's `text-embedding-3-small` model for consistency.
...
- **Trade-offs**: Adds a small latency to queries due to the LLM call, but significantly improves recall by finding relevant documents that might use different terminology.
- **Cost**: One additional LLM API call per search query.
...
- **Trade-offs**: Increases the number of chunks stored (roughly 10-15% more), which means slightly higher storage costs and potentially more results to process.
- **Cost**: No additional API costs, just increased storage usage.
- **Benefits**: Better context preservation, reduced risk of missing relevant information that spans chunk boundaries, improved retrieval for complex topics.
